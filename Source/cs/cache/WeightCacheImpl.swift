...
class WeightCacheImpl : WeightCache, ReflectionProtocol {
    ...
    */
    ...
    fileprivate(set) lazy var pattern: SimplePattern = SimplePattern(REGEX_FLOAT_1MORE)
    
    /**
    * 已有缓存、格式不对、小于等于默认权值的忽略，不加入缓存
    */
    override final func tryCacheKeyValue(_ resourceKey: String, _ resourceValue: String) {
